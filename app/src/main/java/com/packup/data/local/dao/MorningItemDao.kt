...
interface MorningItemDao {
    ...
    @Query("UPDATE morning_items SET status = :status, updatedAt = :now WHERE id = :itemId")
    suspend fun updateStatus(itemId: String, status: MorningItemStatus, now: Long = System.currentTimeMillis())

    @Query("UPDATE morning_items SET name = :name, updatedAt = :now WHERE id = :itemId")
    suspend fun updateName(itemId: String, name: String, now: Long = System.currentTimeMillis())

    @Query("UPDATE morning_items SET category = :newCategory, updatedAt = :now WHERE category = :oldCategory")
